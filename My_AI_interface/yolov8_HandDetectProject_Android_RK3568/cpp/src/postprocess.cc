...
{
    int ret = 0;
    ret = loadLabelName(labels_info, labels);
    if (ret < 0)
    {
    ...
    int samples)
{
    ...
    float step_x = (x2 - x1) / static_cast<float>(samples);
    float step_y = (y2 - y1) / static_cast<float>(samples);

    for (int i = 0; i < samples; ++i)
    {
        for (int j = 0; j < samples; ++j)
        {
            float px = x1 + (i + 0.5f) * step_x;
            float py = y1 + (j + 0.5f) * step_y;

            if (pointInEllipse(px, py,
                               center_x, center_y,
                               axes_w, axes_h))
            {
                inside_count++;
