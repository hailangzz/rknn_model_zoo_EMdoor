...
bool carpet_detect_infer(const cv::Mat& img, std::vector<ObjectCameraDetectResult>& results)
{
    ...
    for (int i = 0; i < od_results.count; i++) {
        ...
        // ===== 新增：edge 采样点 =====
        for (size_t index = 0; index < det->camera_coordinates.add_edge_point_single_pixel_camera_coordinates.size();
            ++index) {
            ...
            one.add_edge_point_single_pixel_camera_coordinates.push_back(dst);
        }
