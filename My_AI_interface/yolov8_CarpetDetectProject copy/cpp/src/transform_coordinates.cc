...
bool CameraParameters::ObjectboxToCameraXYZ(image_rect_t object_rect, box_camera_coordinates &camera_coordinates){
    ...
    // 先清空 vector
    camera_coordinates.add_edge_point_single_pixel_camera_coordinates.clear();
    // 每条边等距取点数（不含端点）
    const int edge_sample_num = 5;

    // 一个小 lambda，专门处理“沿一条边采样”
    auto sample_edge = [&](int x1, int y1, int x2, int y2) {
        for (int i = 1; i <= edge_sample_num; ++i) {
