...
bool base_detect_infer(const cv::Mat &img, std::vector<ObjectCameraDetectResult> &results)
{
    ...
    for (int i = 0; i < od_results.count; i++)
    {
        ...
        filter_mask_contours(contours_mark_point, contours_mark_point_filtered, img_rgb); // 过滤轮廓点集（去除小面积、长宽比异常的轮廓）
        // one.object_contours_mark_point = contours_mark_point;     // 边界点数组赋值

        if (contours_mark_point_filtered.empty()) // 如果过滤后没有有效轮廓点，跳过该目标的后续处理
        {
            continue;
        }
        ...
        ObjectSize3D size;
        float min_width = 0.1f;  // 0.1m 非常近时，线材的宽度、可能会有等于线宽的情况
        float min_height = 0.1f; // 0.1m
