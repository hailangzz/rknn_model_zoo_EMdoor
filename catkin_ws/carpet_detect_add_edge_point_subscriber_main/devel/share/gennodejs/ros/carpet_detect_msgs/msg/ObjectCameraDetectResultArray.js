...
let ObjectCameraDetectResult = require('./ObjectCameraDetectResult.js');
let std_msgs = _finder('std_msgs');

//-----------------------------------------------------------

class ObjectCameraDetectResultArray {
  constructor(initObj={}) {
    if (initObj === null) {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.results.forEach((val) => {
      bufferOffset = ObjectCameraDetectResult.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new ObjectCameraDetectResultArray(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.results[i] = ObjectCameraDetectResult.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    length += 20 * object.results.length;
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new ObjectCameraDetectResultArray(null);
    ...
    if (msg.results !== undefined) {
      ...
      for (let i = 0; i < resolved.results.length; ++i) {
        resolved.results[i] = ObjectCameraDetectResult.Resolve(msg.results[i]);
      }
    }
    else {
      resolved.results = []
...
module.exports = ObjectCameraDetectResultArray;
