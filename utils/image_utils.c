...
/**
 * @brief 根据图像的宽度是否对齐以及是否启用了硬件加速（RGA），选择不同的图像转换方式。如果图像宽度对齐且 RGA 可用，则使用硬件加速进行转换，否则回退到使用 CPU 转换图像
 * @param src_img: 源图像的缓冲区
 * @param dst_img: 目标图像的缓冲区
 * @param src_box: 源图像的矩形区域，定义了要从源图像中转换的部分
 * @param dst_box: 目标图像的矩形区域，定义了要将转换后的图像存放到目标图像的哪个位置
 * @param color: 颜色格式，用于指定图像的颜色格式（如 RGB、YUV 等）
 * @return
 */
int convert_image(image_buffer_t* src_img, image_buffer_t* dst_img, image_rect_t* src_box, image_rect_t* dst_box, char color)
{
...
int convert_image_with_letterbox(image_buffer_t* src_image, image_buffer_t* dst_image, letterbox_t* letterbox, char color)
{
    ...
    // printf("scale=%f dst_box=(%d %d %d %d) allow_slight_change=%d _left_offset=%d _top_offset=%d padding_w=%d padding_h=%d\n",scale, dst_box.left, dst_box.top, dst_box.right, dst_box.bottom, allow_slight_change,_left_offset, _top_offset, padding_w, padding_h);
...
//     memset(out_buf, 0, sizeof(image_buffer_t));

//     out_buf->width  = width;
//     out_buf->height = height;

//     // NV21：每行 Y 数据 width 字节
//     out_buf->width_stride  = width;

//     // YUV420：Y(height) + UV(height/2)
//     out_buf->height_stride = height * 3 / 2;

//     out_buf->format = IMAGE_FORMAT_YUV420SP_NV21;
//     // out_buf->format = IMAGE_FORMAT_RGB888;

//     // 直接指向 camera buffer（不拷贝）
//     out_buf->virt_addr = nv21;

//     // NV21 buffer 大小
//     out_buf->size = width * height * 3 / 2;

//     // 非 DMA / ION buffer
//     out_buf->fd = -1;
// }

void NV21ToRGB(const uint8_t* nv21, int width, int height, uint8_t* rgb)
{
...
void wrap_nv21_to_image_buffer(const uint8_t* nv21,
                                   int width,
                                   int height,
                                   image_buffer_t* out_buf)
{
    if (!nv21 || !out_buf || width <= 0 || height <= 0) {
        return;
    }
    ...
    out_buf->width_stride  = width * 3;
    out_buf->height_stride = height;

    int size = width * height * 3;

    out_buf->virt_addr = (uint8_t*)malloc(size);
    if (!out_buf->virt_addr) {
    ...
    NV21ToRGB(nv21, width, height, out_buf->virt_addr);
}


bool wrap_nv21_to_image_buffer_safe(const uint8_t* nv21,size_t nv21_size,int width,int height,image_buffer_t* out_buf) {
    if (!nv21 || !out_buf || width <= 0 || height <= 0) {
        printf("wrap_nv21: invalid input");
        return false;
    }

    size_t expected = width * height * 3 / 2;
    if (nv21_size < expected) {
        printf("wrap_nv21: nv21_size too small: %zu < %zu",
             nv21_size, expected);
        return false;
    }
    ...
    out_buf->virt_addr = (uint8_t*)malloc(rgb_size);
    if (!out_buf->virt_addr) {
        printf("malloc rgb buffer failed");
        return false;
    }

    out_buf->size = rgb_size;
    out_buf->fd   = -1;
